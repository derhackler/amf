...
class PayloadsInApiCollector(model: BaseUnit) {
  ...
  protected def findCandidates(): Seq[ValidationCandidate] = {
    ...
    val shouldSkip         = mutable.ArrayBuffer[String]()
    ...
    model.iterator().foreach {
      ...
      case union: UnionShape if candidateCollector.keys.exists(_.equals(union.id)) =>
        collectFromExistingShape(candidateCollector, union)
        union.anyOf
          .foreach(member => shouldSkip += member.id) // Skip generate candidates for union members, only union
      ...
      case union: UnionShape =>
        collectFromNewShape(candidateCollector, shapesCollector, union)
        union.anyOf
          .foreach(member => shouldSkip += member.id) // Skip generate candidates for union members, only union
      case shape: AnyShape =>
    ...
    candidateCollector.flatMap {
      case (id, e) if !shouldSkip.contains(id) =>
